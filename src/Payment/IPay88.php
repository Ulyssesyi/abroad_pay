...
use Yijin\AbroadPay\Enums\TransactionType;
...
class IPay88 extends Base
{
     ...
     * @throws Exception
     */
    private function execRequest(TransactionType $type, $params)
    {
        $this->transType = $type;
        $client = new Client([
            'base_uri' => self::DOMAIN,
        ]);
        $res = $client->post(match($this->transType) {
            TransactionType::BARCODE_PAY => '/ePayment/WebService/MHGatewayService/GatewayService.svc',
            TransactionType::QUERY => '/ePayment/Webservice/TxInquiryCardDetails/TxDetailsInquiry.asmx',
            TransactionType::REFUND => '/epayment/webservice/voidapi/voidfunction.asmx',
            default => '',
        }, [
            'body' => $this->buildXMLParams($params),
            'headers' => [
                'Content-Type' => 'text/xml',
                'SOAPAction' => match($this->transType) {
                    TransactionType::BARCODE_PAY => 'https://www.mobile88.com/IGatewayService/EntryPageFunctionality',
                    TransactionType::QUERY => 'https://www.mobile88.com/epayment/webservice/TxDetailsInquiryCardInfo',
                    TransactionType::REFUND => 'https://www.mobile88.com/VoidTransaction',
                    default => '',
                },
            ]
        ]);
